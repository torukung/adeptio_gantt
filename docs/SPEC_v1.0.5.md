...
- **Section delete + action log (user revisions r3+r4, 2026-07-19):** every date divider carries a **bin button** behind the date chip (24px circle, icon flex-centred; subtle at rest, red when armed). Two-click arm-confirm — the first click arms the bin (red) and shows a **glass confirm popover at the RIGHT side of the bin** (apple-glass: `backdrop-filter: blur(14px) saturate(1.6)`, light-red tint `rgba(255,216,226,.55)` light / `rgba(122,20,46,.38)` dark, red ink, 12px radius, fade+scale in; flips to the LEFT of the bin when it would clip the column edge): `ลบโน้ต DD/MM/YYYY ทั้งหมด? — คลิกเพื่อยืนยัน`. Confirm = second bin click OR clicking the popover; auto-disarm ~3.2s removes it. NO bottom toast for arming and NO native confirm() dialogs. Delete removes the WHOLE day section including the divider ("delete all evidences"; today's section clears content and re-hides its divider). Each deletion appends `{ts: ISO, action:'delete', col, date}` to the project's notes **log** (cap 200, trim oldest). The log is viewable via a `log (n)` chip in the popup header that toggles a compact mono strip (`19/07/2026 13:27 · ลบโน้ตธุรกิจ 15/07/2026`); log entries are NOT removable from the UI. **(N10)**
